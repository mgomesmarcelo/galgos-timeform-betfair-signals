...
from src.config import RULE_LABELS, RULE_LABELS_INV, ENTRY_TYPE_LABELS
from src.utils.text import normalize_track_name
...
def main() -> None:
    ...
    # (Sem CSS custom) — Restaurado layout padrão do Streamlit

    # Dimensões padrão para gráficos pequenos (usadas em várias seções)
    small_width = 360
    small_height = 180
    ...
    with col_rule:
        ...
        selected_rule_label = st.selectbox("Regra de seleção", rule_labels, index=0)
    ...
    if df.empty:
        st.info("Nenhum sinal encontrado para a seleção. Gere antes com: python scripts/generate_signals.py --source {src} --market {mkt} --rule {rule} --entry_type both".format(src=source, mkt=market, rule=rule))
        return
    ...
    with col_f3:
        ...
        # (campo movido para a frente do cabeçalho de Stake)
    ...
    filt = df.copy()
    # Filtro adicional: participação do líder (somente para regra líder_volume_total)
    if rule == "lider_volume_total":
        col_l1, col_l2 = st.columns([1, 2])
        with col_l1:
            leader_min = st.number_input(
                "Participação do líder (%) mín.",
                min_value=0.0,
                max_value=100.0,
                value=50.0,
                step=1.0,
    ...
    # Regra principal terceiro_queda50: diferença > 50% em relação ao vol3
    ...
    # Filtro por número de corredores (dinâmico conforme subconjunto atual)
    ...
    # Seletor global do eixo X para gráficos de evolução
    x_axis_mode = st.radio(
        "Eixo X dos gráficos de evolução",
        ["Dia", "Bet"],
        index=0,
        horizontal=True,
        help="Altere entre datas ou sequência de apostas",
    )

    def render_block(title_suffix: str, df_block: pd.DataFrame, entry_kind: str) -> None:
        # ROI e Assertividade, reescalando a partir dos cálculos base (10)
        ...
        # Linha 1: Pistas, Sinais, Greens, Reds, Média BSP, Assertividade
        ...
        with c5:
            ...
            st.metric("Média BSP Alvo", f"{avg_bsp:.2f}")
        with c6:
            st.metric("Assertividade", f"{acc_stake10:.2%}")

        # Campo valor base (reutiliza já existente)
        base_amount = float(st.session_state.get("base_amount", 10.0))
        scale_factor = base_amount / 10.0
        total_base_stake = total_base_stake10 * scale_factor
        total_pnl_stake = total_pnl_stake10 * scale_factor
        ...
        with s1:
            st.metric("Σ Base (Stake)", f"{total_base_stake:.2f}")
        with s2:
        ...
        if entry_kind == "lay":
            ...
            with r1:
                st.metric("Σ Stake (Liability)", f"{total_stake_liab:.2f}")
            with r2:
        ...
        if not plot.empty:
            ...
            if x_axis_mode == "Dia":
                ...
                with st.expander("Evolução Stake (PnL acumulado por dia)", expanded=False):
                ...
                if entry_kind == "lay":
                    with st.expander("Evolução Liability (PnL acumulado por dia)", expanded=False):
                ...
                # Evolução por sequência de apostas (ordem temporal)
                ...
                with st.expander("Evolução Stake (PnL acumulado por bet)", expanded=False):
                    zero_line = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(color="red", strokeWidth=1).encode(y="y:Q")
                    ch = (
                        alt.Chart(plot)
                        .mark_line()
                    ...
                    st.altair_chart(alt.layer(zero_line, ch).configure_view(stroke="#888", strokeWidth=1), use_container_width=True)
                if entry_kind == "lay":
                    with st.expander("Evolução Liability (PnL acumulado por bet)", expanded=False):
                        zero_line2 = alt.Chart(pd.DataFrame({"y": [0]})).mark_rule(color="red", strokeWidth=1).encode(y="y:Q")
                        ch2 = (
                            alt.Chart(plot)
                            .mark_line()
    ...
    if entry_type == "both":
        ...
        # Evita duplicação quando selecionado apenas um tipo: usar apenas os painéis agregados abaixo
        pass
    ...
    # Gráficos de evolução (por dia ou por bet) com opção de minimizar
    plot = filt.copy()
    if not plot.empty:
        plot["ts"] = pd.to_datetime(plot["race_time_iso"], errors="coerce")
        plot = plot.dropna(subset=["ts"]).sort_values("ts")
        # Sempre calcula agregação diária para uso em gráficos semanais, mesmo quando o eixo X é "Bet"
        plot["date_only"] = plot["ts"].dt.date
        daily = plot.groupby("date_only", as_index=False)[["pnl_stake_fixed_10", "pnl_liability_fixed_10"]].sum().sort_values("date_only")
        daily["cum_stake"] = (daily["pnl_stake_fixed_10"] * scale_factor).cumsum()
        daily["cum_liab"] = (daily["pnl_liability_fixed_10"] * scale_factor).cumsum()
        ...
        wd_names = {0: "Seg", 1: "Ter", 2: "Qua", 3: "Qui", 4: "Sex", 5: "Sáb", 6: "Dom"}
    ...
    def _render_small_charts(df_block: pd.DataFrame, entry_kind: str) -> None:
        ...
        if x_axis_mode == "Dia":
            ...
            if not td.empty:
                ...
                st.subheader(f"Evolução por pista (PnL acumulado) - {entry_kind.upper()}")
                base_tracks = (
                    alt.Chart(td)
                    .mark_line()
                    .encode(
            ...
            # Evolução por sequência de apostas dentro de cada pista
        ...
        if "category" in plot2.columns:
            ...
            if not cd.empty:
                ...
                if x_axis_mode == "Dia":
                    ...
                    st.subheader(f"Evolução por categoria (PnL acumulado) - {entry_kind.upper()}")
                    base_cats = (
                        alt.Chart(cd)
                        .mark_line()
                        .encode(
                    ...
                    # Evolução por sequência de apostas dentro de cada categoria
        ...
        if "num_runners" in plot2.columns:
            ...
            if not nd.empty:
                ...
                if x_axis_mode == "Dia":
                    ...
                    st.subheader(f"Evolução por número de corredores (PnL acumulado) - {entry_kind.upper()}")
                    base_nr = (
                        alt.Chart(nd)
                        .mark_line()
                        .encode(
    ...
    # --- Gráficos cruzados: Categoria × Número de corredores (Evolução) ---
    ...
    # --- Gráficos cruzados: Pista × Número de corredores (Evolução) ---
    ...
    # --- Heatmaps por Categoria/Pista × Número de corredores ---
    def _render_heatmaps(df_block: pd.DataFrame, entry_kind: str) -> None:
        base_amount = float(st.session_state.get("base_amount", 10.0))
        scale = base_amount / 10.0
        metric = st.selectbox(
            f"Métrica do heatmap ({entry_kind.upper()})",
        ...
        def _agg(df: pd.DataFrame, by_cols: list[str]) -> pd.DataFrame:
            ...
            return out
        # Categoria × N
        st.subheader(f"Heatmap por categoria × número de corredores - {entry_kind.upper()}")
        ...
        st.subheader(f"Heatmap por pista × número de corredores - {entry_kind.upper()}")
        top_k = 12
    ...
    # Renderização dos novos blocos para BACK/LAY
    if entry_type == "both":
